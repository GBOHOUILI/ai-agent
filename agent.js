...
import {
  runRoadmapCheck,
  findRoadmapFile,
  generateRoadmapFromCode,
  analyzeDependencies,
  getNextTasks,
  extractMarketingPosts,
  STATUTS,
  parseRoadmap,
} from "./modules/roadmap/index.js";
import {
  analyzeProject,
  debugIssue,
  planFeature,
...
function loadProfile() {
  if (fs.existsSync(GLOBAL_PROFILE)) {
    try {
      return JSON.parse(fs.readFileSync(GLOBAL_PROFILE, "utf-8"));
    } catch {
      return null;
    }
  }
...
function printStatus() {
  ...
  p(
    `  Stack    ${C.info}${types.length ? types.join(", ") : "Unknown"}${C.reset}`,
    C.secondary,
  );
  p(
    `  Modèle   ${hasKey ? C.success : C.error}${provider}${C.reset}`,
    C.secondary,
  );
  p(
    `  Thème    ${C.primary}${PROFILE?.theme || "gold"}${C.reset}  Style ${C.primary}${PROFILE?.style || "balanced"}${C.reset}`,
    C.secondary,
  );
  ...
  if (activeEnv) {
    ...
    p(
      `  Config   ${C.error}⚠ Aucune config — tapez 'setup' pour configurer${C.reset}`,
      C.secondary,
    );
  }
  ...
  if (roadmapFiles.length > 0) {
    p(
      `  Roadmap  ${C.primary}${path.basename(roadmapFiles[0])} détecté${C.reset}`,
      C.secondary,
    );
  } else {
    p(
      `  Roadmap  ${C.secondary}aucun — tapez 'roadmap generate'${C.reset}`,
      C.secondary,
    );
  }
  p("");
...
function printHelp() {
  ...
  for (const [section, cmds] of sections) {
    ...
    for (const [cmd, desc] of cmds) {
      p(
        `  ${C.primary}${cmd.padEnd(26)}${C.reset}${C.secondary}${desc}${C.reset}`,
      );
    }
  }
  div();
...
function printProfile() {
  ...
  if (PROFILE.systemPrompt) {
    p(
      `  ${C.primary}Prompt sys  ${C.reset}${C.secondary}${PROFILE.systemPrompt.slice(0, 80)}...${C.reset}`,
    );
  }
  p(`\n  ${C.secondary}Fichier : ${GLOBAL_PROFILE}${C.reset}`);
  div();
  p(
    `  ${C.secondary}Tape 'setup' pour tout reconfigurer avec le wizard.${C.reset}`,
  );
}
...
async function cmdConfig() {
  ...
  for (const line of rawEnv) {
    ...
    p(
      `  ${filled ? C.success + "●" : C.secondary + "○"}  ${key.padEnd(26)} ${filled ? val.slice(0, 10) + "..." : C.secondary + "(vide)"}${C.reset}`,
    );
  }
  if (reloaded > 0) {
    reloadPools();
...
function startSpinner(msg = "Réflexion") {
  ...
  spinnerInterval = setInterval(() => {
    process.stdout.write(
      `\r  ${C.primary}${spinFrames[spinIdx++ % spinFrames.length]}${C.reset}  ${C.secondary}${msg}...${C.reset}`,
    );
  }, 80);
}
...
function displayRoadmapAnalysis(result) {
  ...
  p(
    `\n  Progression  ${C.primary}${bar}${C.reset}  ${C.bold}${pct}%${C.reset}`,
    C.reset,
  );
  p(`  ${summary}`, C.secondary);
  const done = tasks?.filter((t) => t.real_status === "DONE") || [];
  const inProgress =
    tasks?.filter((t) => t.real_status === "IN_PROGRESS") || [];
  const todo = tasks?.filter((t) => t.real_status === "TODO") || [];
  const toUpdate = tasks?.filter((t) => t.should_update) || [];
  p(
    `\n  ✅ Terminées ${C.success}${done.length}${C.reset}   🔄 En cours ${C.primary}${inProgress.length}${C.reset}   ⏳ À faire ${C.secondary}${todo.length}${C.reset}`,
    C.reset,
  );
  if (toUpdate.length > 0) {
    ...
    for (const t of toUpdate.slice(0, 12)) {
      const arrow =
        t.real_status === "DONE"
          ? `${C.success}✓ Terminé${C.reset}`
          : t.real_status === "IN_PROGRESS"
            ? `${C.primary}⟳ En cours${C.reset}`
            : `${C.secondary}○ À faire${C.reset}`;
      p(
        `  ${C.secondary}[${t.id}]${C.reset} ${String(t.titre || "").slice(0, 55)} → ${arrow}`,
      );
      if (t.evidence)
        p(
          `         ${C.secondary}${String(t.evidence).slice(0, 90)}${C.reset}`,
        );
    }
  }
  if (next_priority)
    p(
      `\n  🎯 Prochaine priorité : ${C.primary}${next_priority}${C.reset}`,
      C.reset,
    );
}
...
function cmdRoadmapNext(roadmap) {
  ...
  if (nexts.length === 0) {
    p("\n  ✅ Aucune tâche en attente.", C.success);
    return;
  }
  ...
  for (const t of nexts) {
    const prio =
      {
        CRITIQUE: `${C.error}CRITIQUE`,
        HAUTE: `${C.primary}HAUTE`,
        MOYENNE: `${C.info}MOYENNE`,
        BASSE: `${C.secondary}BASSE`,
      }[t.priorite?.toUpperCase()] || C.secondary + (t.priorite || "?");
    div();
    p(
      `  ${C.primary}[${t.id}]${C.reset} ${C.bold}${t.titre}${C.reset}  |  ${prio}${C.reset}`,
    );
    if (t.description)
      p(`  ${C.secondary}${t.description.slice(0, 120)}${C.reset}`);
    if (t.promptIA) {
      ...
      t.promptIA
        ...
        .map((l) => l.trim())
        .filter(Boolean)
        .forEach((line) => p(`  ${C.secondary}${line}${C.reset}`));
    }
  }
  div();
}
...
function cmdRoadmapDeps(roadmap) {
  ...
  if (conflicts.length > 0) {
    ...
    for (const c of conflicts)
      p(
        `  ${C.error}[${c.task.id}]${C.reset} ${c.task.titre} → dépend de non-terminées: ${c.missingDeps.map((d) => `[${d.id}]`).join(", ")}`,
      );
  }
  if (blocked.length > 0) {
    ...
    for (const b of blocked) {
      ...
      p(
        `     Bloquée par: ${b.blockedBy.map((d) => `${C.secondary}[${d.id}] ${d.titre.slice(0, 40)}${C.reset}`).join(", ")}`,
      );
    }
  }
}
...
function cmdRoadmapMarketing(roadmap, plateforme = "all") {
  ...
  if (posts.length === 0) {
    p(`\n  ℹ  Aucun post marketing trouvé.`, C.secondary);
    return;
  }
  ...
  for (const post of posts) {
    div();
    const statusIcon =
      post.statut === STATUTS.DONE
        ? `${C.success}✅`
        : post.statut === STATUTS.IN_PROGRESS
          ? `${C.primary}🔄`
          : `${C.secondary}⏳`;
    p(
      `  ${C.primary}[${post.id}]${C.reset} ${C.bold}${post.titre}${C.reset}  ${statusIcon}${C.reset}`,
    );
    if (post.linkedin) {
      p(`\n  ${C.info}── LINKEDIN ──${C.reset}`);
      post.linkedin
        .split("\n")
        .slice(0, 8)
        .forEach((l) => p(`  ${l}`, C.secondary));
    }
    if (post.twitter) {
      p(`\n  ${C.info}── TWITTER/X ──${C.reset}`);
      post.twitter
        .split("\n")
        .slice(0, 6)
        .forEach((l) => p(`  ${l}`, C.secondary));
...
async function smartAnswer(input) {
  const context = buildProjectContext(TARGET_DIR, {
    maxFiles: 15,
...
async function planAndExecute(task) {
  ...
  for (const step of plan.steps) {
    ...
    if (step.type === "RUN_CMD" && step.command) {
      ...
      const okRun = await confirm("  Exécuter? (y/n): ");
      if (!okRun) {
        p("  Ignoré.", C.secondary);
        continue;
      }
  ...
  try {
    ...
    if (roadmapFiles.length > 0) {
      ...
      if (nexts.length > 0) {
        ...
        if (t.promptIA) {
          p(
            `  ${C.secondary}→ Tape :${C.reset} ${C.primary}${t.promptIA}${C.reset}`,
          );
        }
        div();
        return;
    ...
    p("", C.reset);
    p(
      `  ${C.secondary}💡 Commandes suggérées :${C.reset}  ${suggestions
        .slice(0, 2)
        .map((s) => `${C.primary}${s}${C.reset}`)
        .join("   ")}`,
      C.reset,
    );
  } catch {
  ...
  try {
    switch (cmd) {
      ...
      case "setup":
        ...
        try {
          execSync(`node ${path.join(__dirname, "setup/wizard.js")}`, {
            stdio: "inherit",
          });
          PROFILE = loadProfile();
      ...
      case "tokens": {
        ...
        p(
          `  ${C.secondary}Ollama local : OLLAMA_MODEL=llama3.2 (défaut)${C.reset}`,
        );
        div();
        break;
      }
      ...
      case "history": {
        ...
        if (!hist.length) {
          p("  Aucun historique pour ce projet.", C.secondary);
          break;
        }
      ...
      case "organize": {
        ...
        if (sub === "go") {
          ...
          if (ok) {
            ...
            await trackCommand("organize", {
              projectStack: detectStack(TARGET_DIR),
            });
          } else {
            p("  Annulé.", C.secondary);
          }
      ...
      case "roadmap": {
        ...
        if (sub === "generate") {
          ...
          if (existing.length > 0) {
            p(
              `\n  ℹ Roadmap existant: ${C.primary}${path.basename(existing[0])}${C.reset}`,
              C.secondary,
            );
            const ok = await confirm(
              "  Générer un nouveau quand même? (y/n): ",
            );
            if (!ok) break;
          }
        ...
        if (sub === "next") {
          ...
          if (!files.length) {
            p("\n  ℹ  Aucun roadmap. Tape 'roadmap generate'.", C.secondary);
            break;
          }
        ...
        if (!result.found) {
          ...
          if (ok) await dispatch("roadmap generate");
          break;
        }
        p(
          `\n  📋 Roadmap : ${C.primary}${result.fileName || path.basename(result.filePath)}${C.reset} · ${result.totalTasks ?? "?"} tâches${result.isEnriched ? ` · ${C.success}format enrichi ✓${C.reset}` : ""}`,
          C.reset,
        );
        if (result.message) {
          p(`\n  ⚠  ${result.message}`, C.primary);
          p(
            `  ${C.secondary}Tape 'roadmap next' pour voir les tâches sans analyse IA.${C.reset}`,
          );
          break;
        }
        div();
        ...
        if (autoUpdate && result.updateResult) {
          p(
            result.updateResult.updated > 0
              ? `\n  ✅ ${result.updateResult.updated} statuts mis à jour`
              : "\n  ℹ Aucun statut à mettre à jour.",
            result.updateResult.updated > 0 ? C.success : C.secondary,
          );
        } else if (
          result.analysis?.tasks?.some((t) => t.should_update) &&
          !autoUpdate
        ) {
          p(
      ...
      case "debug": {
        if (!args) {
          p("  Usage: debug <description du bug>", C.error);
          break;
        }
      ...
      case "build": {
        if (!args) {
          p("  Usage: build <description de la feature>", C.error);
          break;
        }
      ...
      case "refactor": {
        if (!args) {
          p("  Usage: refactor <file> [instruction]", C.error);
          break;
        }
        ...
        if (ok) {
          ...
          p(`  ✅ Refactorisé: ${file}`, C.success);
        }
        break;
      }
      case "generate": {
        if (!args) {
          p("  Usage: generate <filepath> <description>", C.error);
          break;
        }
        const si = args.indexOf(" ");
        if (si === -1) {
          p("  Il faut un chemin ET une description", C.error);
          break;
        }
        ...
        if (ok) {
          writeFile(path.join(TARGET_DIR, file), content);
          p(`  ✅ Créé: ${file}`, C.success);
        }
        break;
      }
      case "tests": {
        if (!args) {
          p("  Usage: tests <file>", C.error);
          break;
        }
        startSpinner("Génération des tests");
        const result = await generateTests(
          path.join(TARGET_DIR, args),
          memory.getHistory(4),
        );
        stopSpinner();
        p(`\n${result}`, C.reset);
        ...
        if (ok) {
          writeFile(path.join(TARGET_DIR, testFile), result);
          p(`  ✅ ${testFile}`, C.success);
        }
        break;
      }
      case "explain": {
        if (!args) {
          p("  Usage: explain <file>", C.error);
          break;
        }
      ...
      case "run": {
        if (!args) {
          p("  Usage: run <commande>", C.error);
          break;
        }
        const safe = isSafeCommand(args);
        if (!safe) {
          const ok = await confirm(
            `  ⚠ Non whitelistée: "${args}". Continuer? (y/n): `,
          );
          if (!ok) break;
        }
      ...
      case "tree":
        p(`\n${scanTree(TARGET_DIR, 4)}`, C.secondary);
        break;
      case "memory": {
        ...
        for (const a of memory.getRecentActions(5)) {
          p(
            `  ${C.secondary}${a.timestamp.slice(0, 16)} ${C.primary}[${a.type}]${C.reset} ${a.description}`,
          );
        }
        break;
      }
      case "git": {
        ...
        if (ok) {
          const r = await run(
            `git add -A && git commit -m "${result.replace(/"/g, "'")}"`,
            { cwd: TARGET_DIR },
          );
      ...
      case "cd": {
        if (!args) {
          p("  Usage: cd <path>", C.error);
          break;
        }
        const np = path.resolve(TARGET_DIR, args);
        if (!fs.existsSync(np)) {
          p(`  ❌ Introuvable: ${np}`, C.error);
          break;
        }
        TARGET_DIR = np;
...
async function main() {
  ...
  if (!hasKey && !PROFILE) {
    ...
    p("");
    p(
      `  ${C.primary}Tape 'setup' pour configurer l'agent en 2 minutes.${C.reset}`,
    );
    p(
      `  ${C.secondary}Clés gratuites : Groq (console.groq.com) · Gemini (aistudio.google.com)${C.reset}`,
    );
    p("");
  } else if (!hasKey) {
  ...
  p(
    `  ${C.secondary}'help' pour les commandes. Question directe = chat avec ${USER_NAME}.${C.reset}`,
  );
  div();
